...
module Duckling.Time.BG.Rules ( rules ) where
...
ruleLastTime = Rule
  { name = "last <time>"
  , pattern =
    [ regex "последн(ият?|ите|ата)|предн(ият?|ите|ата)|минал(ият?|ите|ата)"
    , Predicate isOkWithThisNext
    ]
  , prod = \tokens -> case tokens of
      (_:Token Time td:_) -> tt $ predNth (- 1) False td
...
ruleLastWeekendOfMonth = Rule
  ...
  , pattern =
    [ regex "последният?\\sуикенд\\s(от|на)"
    , Predicate isAMonth
    ]
  , prod = \tokens -> case tokens of
...
-- TODO: Why do we need this when we have ruleDOMLatent?
...
ruleNamedDOMOrdinal = Rule
  { name = "<ordinal> <named-day> | <ordinal> <named-month> (ordinal)"
  , pattern =
    [ Predicate isDOMOrdinal
...
ruleNamedDOMOrdinal2 :: Rule
ruleNamedDOMOrdinal2 = Rule
  { name = "<named-day> <ordinal>"
  , pattern =
    [ Predicate isADayOfWeek
    , Predicate isDOMOrdinal
    ]
...
ruleMonthDOMNumeral = Rule
  { name = "<day-of-month> <named-month> (non ordinal)"
  , pattern =
    [ Predicate isDOMInteger
    , Predicate isAMonth
    ]
...
ruleHHMM = Rule
  { name = "hh:mm"
  , pattern = [regex "((?:[012]?\\d)|(?:2[0-3]))[:.]([0-5]\\d)(ч\\.?)?"]
  , prod = \tokens -> case tokens of
...
ruleHHh = Rule
  ...
  , prod = \case
      (Token RegexMatch (GroupMatch (hh:_)):_) -> do
        h <- parseInt hh
        tt $ hour (h < 12) h
      _ -> Nothing
  }
...
ruleHHMMSS = Rule
  { name = "hh:mm:ss"
  , pattern = [regex "((?:[012]?\\d)|(?:2[0-3]))[:.]([0-5]\\d)[:.]([0-5]\\d)"]
  , prod = \tokens -> case tokens of
...
--     , regex "сутринта|сутрин|предиобед|преди обяд|преди обед|предиобяд"
--     ]
--   , prod = \tokens -> case tokens of
--       (Token RegexMatch (GroupMatch (hh:mm:_)):_:_) -> do
--         h <- parseInt hh
--         m <- parseInt mm
--         tt . timeOfDayAMPM True $ hourMinute True h m
--       _ -> Nothing
--   }
...
--         h <- parseInt hh
--         m <- parseInt mm
--         tt . timeOfDayAMPM False $ hourMinute True h m
--       _ -> Nothing
--   }
...
--     ]
--   , prod = \tokens -> case tokens of
--       (Token RegexMatch (GroupMatch (hh:_)):_:_) -> do
--         h <- parseInt hh
--         tt . timeOfDayAMPM True $ hour True h
--       _ -> Nothing
--   }
...
--       (Token RegexMatch (GroupMatch (hh:_)):_:_) -> do
--         h <- parseInt hh
--         tt . timeOfDayAMPM False $ hour True h
--       _ -> Nothing
--   }

-- spelledOutAM :: Rule
-- spelledOutAM = Rule
--   { name = "spelled out numbers am"
--   , pattern =
--     [ Predicate $ isIntegerBetween 1 12
--     , Predicate $ isIntegerBetween 1 59
--     , regex "сутринта|сутрин|предиобед|преди обед|преди обяд"
--     ]
--     , prod = \tokens -> case tokens of
--         (h:m:Token RegexMatch (GroupMatch (_:ap:_)):_) -> do
--           hh <- getIntValue h
--           mm <- getIntValue m
--           tt . timeOfDayAMPM True $ hourMinute True hh mm
--         _ -> Nothing
--   }

-- spelledOutPM :: Rule
-- spelledOutPM = Rule
--   { name = "spelled out numbers pm"
--   , pattern =
--     [ Predicate $ isIntegerBetween 1 12
--     , Predicate $ isIntegerBetween 1 59
--     , regex "вечерта|вечер|следобед|след обяд|след обед"
--     ]
--     , prod = \tokens -> case tokens of
--         (h:m:Token RegexMatch (GroupMatch (_:ap:_)):_) -> do
--           hh <- getIntValue h
--           mm <- getIntValue m
--           tt . timeOfDayAMPM False $ hourMinute True hh mm
--         _ -> Nothing
--   }

-- spelledOutHourAM :: Rule
-- spelledOutHourAM = Rule
--   { name = "spelled out numbers am"
--   , pattern =
--     [ Predicate $ isIntegerBetween 1 12
...
--     ]
--     , prod = \tokens -> case tokens of
--         (h:Token RegexMatch (GroupMatch (_:ap:_)):_) -> do
--           hh <- getIntValue h
--           tt . timeOfDayAMPM True $ hour True hh
--         _ -> Nothing
--   }

-- spelledOutHourPM :: Rule
-- spelledOutHourPM = Rule
--   { name = "spelled out numbers pm"
--   , pattern =
--     [ Predicate $ isIntegerBetween 1 12
...
--         (h:Token RegexMatch (GroupMatch (_:ap:_)):_) -> do
--           hh <- getIntValue h
--           tt . timeOfDayAMPM False $ hour True hh
--         _ -> Nothing
--   }
...
ruleHODurationBefore :: Rule
ruleHODurationBefore = Rule
  { name = "<hour-of-day> без <duration>"
  , pattern =
    [ Predicate isAnHourOfDay
    , regex "без"
    , Predicate $ isDurationLessThan TG.Hour
...
ruleDDMMYYYY = Rule
  { name = "dd-mm-yyyy"
  , pattern =
    [ regex "(3[01]|[12]\\d|0?[1-9])[\\.\\/\\-](0?[1-9]|1[0-2])[\\.\\/\\-](\\d{2,4})(г\\.?)?"
    ]
  , prod = \case
...
ruleAfterPartofday = Rule
  ...
  , pattern =
    [ regex "след (закуска|вечеря|работа|училище)"
    ]
  , prod = \tokens -> case tokens of
      (Token RegexMatch (GroupMatch (match:_)):_) -> do
...
ruleWeek = Rule
 { name = "week"
 , pattern = [regex "((цяла|цялата|останалата) седмица|седмицата)"]
 , prod = \case
     (Token RegexMatch (GroupMatch (match:_)):_) ->
       let end = cycleNthAfter True TG.Day (-2) $ cycleNth TG.Week 1
           period = case Text.toLower match of
                      "цяла седмица" -> interval Closed (cycleNth TG.Week 0) end
                      "цялата седмица" -> interval Closed (cycleNth TG.Week 0) end
                      "останалата седмица" -> interval Open today end
                      "седмицата" -> interval Open today end
                      _ -> Nothing
       in case Text.toLower match of
         "седмицата" -> Token Time . mkLatent <$> period
         _ -> Token Time <$> period
     _ -> Nothing
 }
...
ruleSeasons = mkRuleSeasons
  ...
  , ( "the summer", "лятото"   , monthDay  6 21, monthDay  9 23 )
  , ( "fall"      , "есен"     , monthDay  9 23, monthDay 12 21 )
  , ( "the fall"  , "есента"   , monthDay  9 23, monthDay 12 21 )
  , ( "winter"    , "зима"     , monthDay 12 21, monthDay  3 20 )
  , ( "the winter", "зимата"   , monthDay 12 21, monthDay  3 20 )
  , ( "spring"    , "пролет"   , monthDay  3 20, monthDay  6 21 )
  , ( "the spring", "пролетта" , monthDay  3 20, monthDay  6 21 )
  ]

ruleTODPrecision :: Rule
ruleTODPrecision = Rule
  { name = "exactly|approximately <time-of-day>"
  , pattern =
    [ Predicate isATimeOfDay
    , regex "(точно|точно в|около)"
    ]
  , prod = \tokens -> case tokens of
      (Token Time td:_) -> tt $ notLatent td
      _ -> Nothing
...
ruleIntervalTODFrom = Rule
  ...
  , pattern =
    [ regex "след|от"
    , Predicate isATimeOfDay
    , regex "((,\\s+но )?(преди|по-рано от))|\\-|до"
    , Predicate isATimeOfDay
    ]
  , prod = \tokens -> case tokens of
      (_:Token Time td1:_:Token Time td2:_) ->
...
ruleIntervalUntilTime = Rule
  { name = "until <time>"
  , pattern =
    [ regex "(по (някое|всяко) време |понякога )?(преди|до)"
    , dimension Time
    ]
  , prod = \tokens -> case tokens of
...
ruleIntervalAfterFromSinceTime = Rule
  ...
  , pattern =
    [ regex "след|от|(по (някое|всяко) време |понякога )?след"
    , dimension Time
    ]
  , prod = \tokens -> case tokens of
...
ruleDaysOfWeek = mkRuleDaysOfWeek
  [ ( "Monday"   , "понеделник|пон\\.?"     )
  , ( "Tuesday"  , "вторник|вт(о|орн)?\\.?" )
  , ( "Wednesday", "сряда|сря?\\.?"         )
  , ( "Thursday" , "четвъртък|четв?\\.?"    )
  , ( "Friday"   , "петък|пет\\.?"         )
  , ( "Saturday" , "събота|съб\\.?"        )
  , ( "Sunday"   , "неделя|нед\\.?"        )
  ]
...
ruleMonths = mkRuleMonthsWithLatent
  [ ( "January"  , "януари|ян\\.?"      , False )
  , ( "February" , "февруари|февр?\\.?" , False )
  , ( "March"    , "март|мар\\.?"       , False )
  , ( "April"    , "април|апр\\.?"      , False )
  , ( "May"      , "май"                , True  )
  , ( "June"     , "юни"                , False )
  , ( "July"     , "юли"                , False )
  , ( "August"   , "август|авг\\.?"     , False )
  , ( "September", "септември|септ?\\.?", False )
  , ( "October"  , "октомври|окт\\.?"   , False )
  , ( "November" , "ноември|ноем?\\.?"  , False )
  , ( "December" , "декември|дек\\.?"   , False )
  ]
...
ruleEndOfMonth = Rule
  { name = "end of month"
  , pattern = [ regex "(до |в )?(краят? на (месеца|месецът))" ]
  , prod = \tokens -> case tokens of
      (Token RegexMatch (GroupMatch (match:_)):_)
...
ruleBeginningOfMonth = Rule
  ...
  , pattern = [ regex "(в )?(началото на месец(а|ът))" ]
  , prod = \_ -> do
...
ruleEndOfYear = Rule
  { name = "end of year"
  , pattern = [ regex "(до |в )?(краят? на годината)" ]
  , prod = \tokens -> case tokens of
      (Token RegexMatch (GroupMatch (match:_)):_) -> do
        start <- std
...
ruleBeginningOfYear = Rule
  ...
  , pattern = [ regex "(в )?(началото на годината)" ]
  , prod = \_ -> do
  ...
  , prod = \tokens -> case tokens of
      (Token RegexMatch (GroupMatch (match:_)):Token TimeGrain grain:_) ->
        case Text.toLower match of
          ...
          "идващият"      -> tt $ cycleNth grain 0
          "идващите"      -> tt $ cycleNth grain 0
          "идващата"      -> tt $ cycleNth grain 0
          "текущия"       -> tt $ cycleNth grain 0
          "текущият"      -> tt $ cycleNth grain 0
          "текущите"      -> tt $ cycleNth grain 0
          "текущата"      -> tt $ cycleNth grain 0
          "последния"     -> tt . cycleNth grain $ - 1
          "последният"    -> tt . cycleNth grain $ - 1
          "последните"    -> tt . cycleNth grain $ - 1
          "последната"    -> tt . cycleNth grain $ - 1
          "миналия"       -> tt . cycleNth grain $ - 1
          "миналият"      -> tt . cycleNth grain $ - 1
          "миналите"      -> tt . cycleNth grain $ - 1
          "миналата"      -> tt . cycleNth grain $ - 1
          "предходния"    -> tt . cycleNth grain $ - 1
          "предходният"   -> tt . cycleNth grain $ - 1
          "предходните"   -> tt . cycleNth grain $ - 1
          "предходната"   -> tt . cycleNth grain $ - 1
          "предния"       -> tt . cycleNth grain $ - 1
          "предният"      -> tt . cycleNth grain $ - 1
          "предните"      -> tt . cycleNth grain $ - 1
          "предната"      -> tt . cycleNth grain $ - 1
          "следващия"     -> tt $ cycleNth grain 1
          "следващият"    -> tt $ cycleNth grain 1
          "следващите"    -> tt $ cycleNth grain 1
          "следващата"    -> tt $ cycleNth grain 1
          "другия"        -> tt $ cycleNth grain 1
...
ruleIntervalForDurationFrom = Rule
  ...
  , pattern =
    [ regex "за"
    , dimension Duration
    , regex "(от|започвайки от|започващи от|след)"
    , dimension Time
    ]
  , prod = \tokens -> case tokens of
...
--   , ruleTheDOMNumeral
  ...
  , ruleDOMLatent
  , ruleNamedDOMOrdinal
  , ruleNamedDOMOrdinal2
  , ruleNamedDOMOrdinal3
  , ruleMonthDOMNumeral
  , ruleDOMMonth
--   , ruleDOMOfMonth
